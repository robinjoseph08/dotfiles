...
A skill available for automatic agent discovery because `disable-model-invocation` is absent or false. Its **description** stays in the agent's context as a trigger, so the model can select it without an explicit path. The human can still invoke it directly. This discoverability creates permanent **context load**.

_Avoid_: ability, tool, capability

### User-Invoked

A skill hidden from automatic discovery with `disable-model-invocation: true`. It keeps a human-facing **description** for command menus but contributes no automatic-discovery **context load**. A human can invoke it directly, and another skill can reach it through an explicit **context pointer** to its file.

_Avoid_: procedure, workflow, command

### Description

The required summary of a skill. For a **model-invoked** skill it is also a machine-readable trigger held in the agent's context. For a **user-invoked** skill it is human-facing metadata hidden from automatic discovery. The `disable-model-invocation` field, not description presence, selects the invocation mode.
...
The cost a **model-invoked** skill imposes on the agent's context window: its **description**, always loaded, spending both tokens and attention. What **user-invoked** skills escape by hiding their description from automatic discovery, and the brake on splitting into more model-invoked skills.
...
A **user-invoked** skill whose job is to point at other user-invoked skills, naming each and when to reach for it, so the human has one skill to remember instead of many. It may direct the agent through explicit **context pointers**, even though those skills are absent from automatic discovery. The cure for **cognitive load** when user-invoked skills multiply.
...
**Reference** that lives outside the skill system: a plain file with no **description**, no **steps**, and no invocation command that any skill can point at. It is a useful shared home for material that does not need to fire on its own.
